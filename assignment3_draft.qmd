...
# silhouette scores for k-means (K=2 to K=20)
silhouette_kmeans <- function(data, max_k = 20) {
  sil_scores <- numeric(max_k - 1)  # Store silhouette scores
...
sil_scores_kmeans <- silhouette_kmeans(scaled_data, max_k = 20)

# Plot silhouette scores for k-means
plot(2:20, sil_scores_kmeans, type = "b", pch = 19, col = "blue", 
...
```{r}
library(cluster)
# Function to calculate silhouette scores for k-medoids (PAM) (K=2 to K=20)
pam_fn <- function(data, max_k = 20) {
  ...
  for (k in 2:max_k) {
    ...
    # Calculate silhouette score
    sil <- silhouette(pam_res$clustering, dist(data))
    sil_scores[k - 1] <- mean(sil[, 3])  # Average silhouette score
  }
...
sil_scores_pam <- pam_fn(scaled_data, max_k = 20)


plot(2:20, sil_scores_pam, type = "b", pch = 19, col = "red", 
     xlab = "Number of Clusters (K)", ylab = "Average Silhouette Score", 
     main = "Silhouette Plot for K-medoids")
